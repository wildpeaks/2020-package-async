...
describe('Invalid values', () => {
	it('undefined', async() => {
		...
		let results = [];
		try {
			results = await asyncArrayMap(undefined, makeUppercase); // eslint-disable-line no-undefined
		} catch(e){
			error = e.message;
		}
...
describe('Invalid callback', () => {
	it('undefined', async() => {
		...
		let results = [];
		try {
			results = await asyncArrayMap(['Aaa', 'Bbb', 'Ccc'], undefined); // eslint-disable-line no-undefined
		} catch(e){
			error = e.message;
		}
		expect(results).toEqual([]);
